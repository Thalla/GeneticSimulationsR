...
livingTranslationTypes <- c(nrow(configLiving[which(configLiving$translationType == "random"),]), nrow(configLiving[which(configLiving$translationType == "affinity"),]))
deadTranslationTypes <- c(nrow(configDead[which(configDead$translationType == "random"),]), nrow(configDead[which(configDead$translationType == "affinity"),]))

matrix = matrix(rbind(livingTranslationTypes, deadTranslationTypes),nrow = 2)
colnames(matrix)=c("random", "affinity")
rownames(matrix) = c("living", "dead")
data_percentage=apply(matrix,2, function(x){x*100/sum(x,na.rm=T)})
...
```{r big ANOVA eval=FALSE, include=FALSE}
...
interaction(c(16, 19, 20, 21, 23, 24, 25))

conf <- lnconfig
interact <- interaction(factor(conf[[1]]), factor(conf[[2]]), factor(conf[[3]]), factor(conf[[4]]), factor(conf[[5]]), factor(conf[[6]]), factor(conf[[7]]))

conf <- configLiving[c(1,16, 19, 20, 21, 23, 24, 25)]
...
aovResult <- aov(lengths ~ interact)
aovResult
summary(aovResult)

interact <- interaction(factor(configLiving[,16]), factor(configLiving[,19]), factor(configLiving[,20]), factor(configLiving[,21]), factor(configLiving[,23]), factor(configLiving[,24]), factor(configLiving[,25]))
...
```{r reducedConfig for survival rate}
...
# @Before Set config variable. Comment/uncomment unique(reducedConfig) line and decide between means and lengths or both.
  config <- configLiving
  reducedConfig <- configLiving[,c(16, 19:21, 23:25, 1)]
 ...
 counts
 # t.test: H0: Treatments with 10 surviving cells are not significantly different from a set of treatments with 10 and 9 surviving cells togeter. Result: They are different. Problem: The higher the observation number the higher the probability for significance. 
 ten <- cbind(1:826, 10)
 nine <- cbind(1:61, 9)
 nineten <- rbind(nine, ten)
 t.test(nineten, ten)
 ...
 # use the lists of means to check if they are normally distributed
  ...
  # Choose all rows of treatments and the normality p values that have 10 surviving cells
 lconfig <- configLiving[lengthsOfOutputMeans == 10][,c(16, 19:21, 23:25, 1)]
 normalityTestPValue <- normalityTestPValue[lengthsOfOutputMeans == 10]
 
   # Get the normally distributed values
  lnconfig <- lconfig[normalityTestPValue > 0.05]
  lnonNormConfig <- lconfig[normalityTestPValue <= 0.05]
  
  # check the homogeneity for the normally distributed values
...
HomogeneityPValue <- bartlett.test(lnconfig$mean ~ interact)

# What about not normally distributed values?
 
  # ......

 
# normal distribution but no homogeneity of variance
result <- oneway.test(lnconfig$mean~interact)
result
summary(result)
res <- pairwise.t.test(lnconfig$mean, interact, pool.sd = FALSE)


# @Before lnconfig <- data to be analysed (only relevant rows plus mean at the end)

analysis1 = function (paramIndex){
  param <- lnconfig[[paramIndex]]
  reducedConfig <- lnconfig[,..paramIndex]
  reducedConfig <- unique(reducedConfig)
  
  print(summary(factor(param)))
  
  summarize <- function (p1){
   config$mean[which(param == p1)]
  }
  
  outputMeansOrdered <- sapply(1:nrow(reducedConfig), FUN = function(pos){summarize(reducedConfig[[1]][pos])})
  ...
  print(normalityTestPValue)
  
  if(length(normalityTestPValue[which(normalityTestPValue > 0.05)]) == length(normalityTestPValue)){
    HomogeneityPValue <- bartlett.test(lnconfig$mean ~ as.factor(param))$p.value
    
    print("Homogeneity of Variance")
    print(HomogeneityPValue)
    
    if(HomogeneityPValue <= 0.05){
      oneway <- oneway.test(lnconfig$mean~factor(param))
      
      print("oneway")
      print(oneway$p.value)
      
      print("Post hoc")
      posthoc <- pairwise.t.test(lnconfig$mean, factor(param), pool.sd = FALSE)
      print(pander(posthoc$p.value))
...
sapply(1:7, FUN = function (pos){analysis1(pos)})
...
library(modelr)
 ...
 posthoc <- pairwise.wilcox.test(lnconfig$mean , factor(lnconfig$initAaNumb):factor(lnconfig$geneNumb), p.adjust.method = "bonferroni")
 posthoc <- pairwise.t.test(lnconfig$mean, lnconfig$aarsLifeticksStartValue, pool.sd = FALSE)
...
library(rcompanion)
 ...
 multcompLetters(posthoc, compare="<", threshold = 0.05, Letters = letters, reversed = FALSE)
